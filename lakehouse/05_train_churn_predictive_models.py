...
# DBTITLE 1,Load Customer Data and Define Churn Labels
# MAGIC %sql
# MAGIC -- Create training dataset with churn labels
# MAGIC -- Churn = customer who was active and became inactive
# MAGIC -- Use 30-day window: if days_since_last_purchase > 90, consider churned
# MAGIC
# MAGIC CREATE OR REPLACE TEMP VIEW customer_churn_labels AS
# MAGIC SELECT 
# MAGIC     c360.customer_id,
# MAGIC     c360.customer_status,
# MAGIC     c360.customer_segment,
# MAGIC     c360.loyalty_tier,
# MAGIC     c360.country,
# MAGIC     c360.region,
# MAGIC     crm.age_band,
# MAGIC     crm.preferred_channel,
# MAGIC     
# MAGIC     -- Revenue & transaction metrics
# MAGIC     c360.total_revenue,
# MAGIC     c360.number_of_transactions,
# MAGIC     c360.average_order_value,
# MAGIC     c360.days_since_last_purchase,
# MAGIC     c360.purchases_last_30_days,
# MAGIC     c360.purchases_last_60_days,
# MAGIC     c360.purchases_last_90_days,
# MAGIC     
# MAGIC     -- Engagement metrics
# MAGIC     c360.marketing_engagement_rate,
# MAGIC     c360.average_satisfaction_score,
# MAGIC     c360.service_interactions,
# MAGIC     c360.unresolved_interactions,
# MAGIC     
# MAGIC     -- Behavioral change signals (key for churn prediction)
# MAGIC     c360.revenue_change_pct,
# MAGIC     c360.purchase_frequency_change_pct,
# MAGIC     c360.avg_basket_change_pct,
# MAGIC     
# MAGIC     -- Churn label: inactive OR days_since_last_purchase > 90
# MAGIC     CASE 
# MAGIC         WHEN c360.customer_status = 'inactive' OR c360.days_since_last_purchase > 90 THEN 1
# MAGIC         ELSE 0
# MAGIC     END as is_churned,
# MAGIC     
# MAGIC     -- Time to churn (for survival model): capped at 365 days
# MAGIC     LEAST(c360.days_since_last_purchase, 365) as days_to_event,
# MAGIC     
# MAGIC     -- Event indicator for survival analysis
# MAGIC     CASE 
# MAGIC         WHEN c360.customer_status = 'inactive' OR c360.days_since_last_purchase > 90 THEN 1
# MAGIC         ELSE 0
# MAGIC     END as event_observed
# MAGIC     
# MAGIC FROM workspace.retail.gold_customer_360 c360
# MAGIC LEFT JOIN workspace.retail.bronze_crm_customer crm
# MAGIC     ON c360.customer_id = crm.customer_id
# MAGIC WHERE crm.age_band IS NOT NULL;
# MAGIC
# MAGIC SELECT 
# MAGIC     COUNT(*) as total_customers,
# MAGIC     SUM(is_churned) as churned_customers,
# MAGIC     ROUND(AVG(is_churned) * 100, 2) as churn_rate_pct
# MAGIC FROM customer_churn_labels;
...
# DBTITLE 1,Create Customer State History Table
# MAGIC %sql
# MAGIC -- Create state history table to track semaphore transitions
# MAGIC CREATE TABLE IF NOT EXISTS workspace.retail.customer_state_history (
# MAGIC     customer_id STRING,
# MAGIC     previous_state STRING,
# MAGIC     current_state STRING,
# MAGIC     transition_date TIMESTAMP,
# MAGIC     churn_probability DOUBLE,
# MAGIC     days_to_churn DOUBLE,
# MAGIC     trigger_event STRING COMMENT 'What caused the state change',
# MAGIC     model_version STRING
# MAGIC )
# MAGIC USING DELTA
# MAGIC COMMENT 'Tracks customer risk state transitions over time for monitoring detection quality';
# MAGIC
# MAGIC -- Initialize with current states
# MAGIC INSERT INTO workspace.retail.customer_state_history
# MAGIC SELECT 
# MAGIC     customer_id,
# MAGIC     NULL as previous_state,
# MAGIC     semaphore_state as current_state,
# MAGIC     prediction_date as transition_date,
# MAGIC     churn_probability,
# MAGIC     predicted_days_to_churn as days_to_churn,
# MAGIC     'initial_state' as trigger_event,
# MAGIC     model_version
# MAGIC FROM workspace.retail.risk_predictions;
# MAGIC
# MAGIC SELECT 
# MAGIC     COUNT(*) as total_records,
# MAGIC     COUNT(DISTINCT customer_id) as unique_customers
# MAGIC FROM workspace.retail.customer_state_history;

# COMMAND ----------

# DBTITLE 1,Create Intervention Recommendations Table
# MAGIC %sql
# MAGIC -- Table for recommended actions per customer
# MAGIC CREATE TABLE IF NOT EXISTS workspace.retail.intervention_recommendations (
# MAGIC     customer_id STRING,
# MAGIC     recommended_action STRING COMMENT 'Call, Training, Support, Discount, Product engagement, No intervention',
# MAGIC     action_priority INT COMMENT 'Ranking of urgency',
# MAGIC     expected_success_probability DOUBLE COMMENT 'From uplift model',
# MAGIC     recommended_channel STRING,
# MAGIC     recommended_timing STRING,
# MAGIC     business_justification STRING,
# MAGIC     recommendation_date TIMESTAMP,
# MAGIC     model_version STRING
# MAGIC )
# MAGIC USING DELTA
# MAGIC COMMENT 'AI-recommended interventions for at-risk customers';
# MAGIC
# MAGIC -- Generate initial recommendations based on risk + uplift
# MAGIC INSERT INTO workspace.retail.intervention_recommendations
# MAGIC SELECT 
# MAGIC     rp.customer_id,
# MAGIC     CASE 
# MAGIC         WHEN rp.churn_probability > 0.75 AND rp.uplift_score > 0.3 THEN 'Call'
# MAGIC         WHEN rp.churn_probability > 0.5 AND rp.uplift_score > 0.2 THEN 'Discount'
# MAGIC         WHEN rp.churn_probability > 0.5 AND c360.service_interactions > 5 THEN 'Support'
# MAGIC         WHEN rp.churn_probability > 0.25 AND rp.uplift_score > 0.15 THEN 'Product engagement'
# MAGIC         WHEN rp.churn_probability < 0.25 THEN 'No intervention'
# MAGIC         ELSE 'Training'
# MAGIC     END as recommended_action,
# MAGIC     rp.priority_rank as action_priority,
# MAGIC     rp.uplift_score as expected_success_probability,
# MAGIC     crm.preferred_channel as recommended_channel,
# MAGIC     CASE 
# MAGIC         WHEN rp.predicted_days_to_churn < 30 THEN 'Urgent (within 7 days)'
# MAGIC         WHEN rp.predicted_days_to_churn < 90 THEN 'Soon (within 14 days)'
# MAGIC         ELSE 'Scheduled (within 30 days)'
# MAGIC     END as recommended_timing,
# MAGIC     CASE 
# MAGIC         WHEN rp.churn_probability > 0.75 THEN 'Critical risk - immediate action required'
# MAGIC         WHEN rp.uplift_score > 0.3 THEN 'High response probability - intervention likely effective'
# MAGIC         WHEN rp.is_high_value = 1 THEN 'High-value customer - retention priority'
# MAGIC         ELSE 'Standard monitoring'
# MAGIC     END as business_justification,
# MAGIC     rp.prediction_date as recommendation_date,
# MAGIC     rp.model_version
# MAGIC FROM workspace.retail.risk_predictions rp
# MAGIC LEFT JOIN workspace.retail.gold_customer_360 c360 ON rp.customer_id = c360.customer_id
# MAGIC LEFT JOIN workspace.retail.bronze_crm_customer crm ON rp.customer_id = crm.customer_id
# MAGIC WHERE rp.churn_probability > 0.25;  -- Only recommend for medium+ risk
# MAGIC
# MAGIC SELECT 
# MAGIC     recommended_action,
# MAGIC     COUNT(*) as customer_count,
# MAGIC     ROUND(AVG(expected_success_probability), 3) as avg_success_prob
# MAGIC FROM workspace.retail.intervention_recommendations
# MAGIC GROUP BY recommended_action
# MAGIC ORDER BY customer_count DESC;

# COMMAND ----------

# DBTITLE 1,Create Intervention Outcomes Table
# MAGIC %sql
# MAGIC -- Table to track what happened after interventions
# MAGIC CREATE TABLE IF NOT EXISTS workspace.retail.intervention_outcomes (
# MAGIC     customer_id STRING,
# MAGIC     intervention_date TIMESTAMP,
# MAGIC     action_taken STRING,
# MAGIC     action_channel STRING,
# MAGIC     initial_risk_state STRING,
# MAGIC     initial_churn_probability DOUBLE,
# MAGIC     
# MAGIC     -- Outcome tracking (filled in after 30/60/90 days)
# MAGIC     outcome_date TIMESTAMP,
# MAGIC     final_risk_state STRING,
# MAGIC     final_churn_probability DOUBLE,
# MAGIC     customer_retained BOOLEAN COMMENT 'Did customer remain active?',
# MAGIC     revenue_after_30d DOUBLE,
# MAGIC     revenue_after_60d DOUBLE,
# MAGIC     revenue_after_90d DOUBLE,
# MAGIC     
# MAGIC     -- Diagnostic flags
# MAGIC     detection_failure BOOLEAN COMMENT 'Did we miss early warning signs?',
# MAGIC     intervention_failure BOOLEAN COMMENT 'Did intervention not work?',
# MAGIC     timing_failure BOOLEAN COMMENT 'Did we act too late?',
# MAGIC     
# MAGIC     model_version STRING
# MAGIC )
# MAGIC USING DELTA
# MAGIC COMMENT 'Tracks outcomes of interventions for model monitoring and improvement';
# MAGIC
# MAGIC SELECT 'Table created: workspace.retail.intervention_outcomes' as status;
